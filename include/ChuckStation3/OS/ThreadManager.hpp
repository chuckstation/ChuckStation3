...
public:
    ThreadManager(PlayStation3* ps3) : ps3(ps3) {
        // Avoid reallocations
        // My code relies on this... I know it's bad. I pass pointers to Thread objects to the scheduler, if we
        // reallocate the pointers will become invalid
        // TODO: definitely find a better way to do this
        threads.reserve(256); // More than enough
    }
    PlayStation3* ps3;

    Thread*             createThread(u64         entry,
                                     u64         stack_size,
                                     u64         arg,
                                     s32         prio,
                                     const u8*   name,
