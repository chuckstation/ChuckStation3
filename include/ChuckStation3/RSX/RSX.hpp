...
public:
    ...
    void runCommandList();
    bool doCmd(u32 cmd_num, std::deque<u32>& args);
    u32  fetch32();
    ...
    static constexpr char swizzle_str_map[] = {'a', 'r', 'g', 'b'};
    ...
    // Immediate vertex data (uploaded via the SET_VERTEX_DATA_* set of commands) can be use for OpenGL immediate-mode
    // style drawing, but it can also be mixed with normal drawing that uses vertex arrays. For example, you can enable
    // vertex attributes 0 and 1 in the vertex array, and then upload attribute 2 via immediate data. Immediate data is
    // meant for immediate drawing only if the data is uploaded inside a BEGIN/END block. The flag below is set when
    // that is the case.
    ...
    void compileProgram();
    void setupVAO();
    template <bool is_inline_array = false>
