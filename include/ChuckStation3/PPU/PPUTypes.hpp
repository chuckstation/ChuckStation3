...
struct ConditionRegister {
    u32 raw = 0;
    enum { LESS = 0b1000, GREATER = 0b0100, EQUAL = 0b0010 };
    ...
    void compareAndUpdateCRField(u8 n, T a, T b, u8 so = 0) {
        if (a < b)
            setCRField(n, LESS, so);
        else if (a > b)
            setCRField(n, GREATER, so);
        else
            setCRField(n, EQUAL, so);
    }
};
...
struct XER {
    ...
    bool ov      = false;
    bool so      = false;
    u64  get() { return (so << 31) | (ov << 30) | (ca << 29) | (n_bytes & 0x7f); }
};
...
static const u64 lvsl_shifts[0x10][2] = {{0x08090A0B0C0D0E0F, 0x0001020304050607},
...
enum G_04Opcodes {
    ...
    VCMPGEFP_  = 0x5c6,
    VLOGEFP    = 0x1ca, // Vector Log2 Estimate Floating-Point
    ...
    VCMPGTUB_  = 0x606,
    VMULEUB    = 0x208,
    VRFIN      = 0x20a, // Vector Round to Floating-Point Integer Nearest
    VSPLTB     = 0x20c, // Vector Splat Byte
    VUPKHSB    = 0x20e, // Vector Unpack High Signed Byte
    VADDUHS    = 0x240,
    VMINUH     = 0x242,
    VSRH       = 0x244, // Vector Shift Right Half Word
