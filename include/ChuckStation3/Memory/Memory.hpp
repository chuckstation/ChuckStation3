...
public:
    ...
    u8*                 getPtr(u64 vaddr);
    void                copyTo(u64 dst, u8* src, size_t size);
    void                copyFrom(u8* dst, u64 src, size_t size);
    ...
    MemoryRegion::MapEntry* alloc(size_t size, u64 start_addr = 0, bool system = false, u64 alignment = PAGE_SIZE) {
        return ram.alloc(size, start_addr, system, alignment);
    }
    ...
    std::pair<bool, MemoryRegion::MapEntry*> findNextMappedArea(u64 start_addr) {
        return ram.findNextMappedArea(start_addr);
    }
    u64 findNextAllocatableVaddr(size_t size, u64 start_addr = 0, u64 alignment = PAGE_SIZE) {
        return ram.findNextAllocatableVaddr(size, start_addr, alignment);
    }
    std::pair<bool, MemoryRegion::MapEntry*> findMapEntryWithHandle(u64 handle) {
        return ram.findMapEntryWithHandle(handle);
    }
    std::pair<bool, MemoryRegion::MapEntry*> isMapped(u64 vaddr) { return ram.isMapped(vaddr); }
    MemoryRegion::MapEntry*                  mmap(u64 vaddr, u64 paddr, size_t size, bool fastmem = true) {
        return ram.mmap(vaddr, paddr, size, fastmem);
    }
