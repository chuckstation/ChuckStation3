...
private:
    ...
    std::unordered_map<u64, std::function<void(u64)>>* getWatchpointList(const MemoryWatchpoint& watchpoint,
                                                                         bool&                   is_read_watchpoint) {
        is_read_watchpoint = isReadWatchpoint(watchpoint);

        if (is_read_watchpoint)
            return &ps3->mem.watchpoints_r;
        else
            return &ps3->mem.watchpoints_w;
    }
