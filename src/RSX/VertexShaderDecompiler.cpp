...
uniform ivec2 surface_clip;
    ...
    for (int i = start * 4; i < 512 * 4; i += 4) {
        ...
        // They can be an input vector (i.e. "in_pos"), a data vector (each shader has 16 general purpose vectors to
        // work with), or a constant register (uniforms)
        ...
        switch (instr->w1.vector_opc) {
            ...
            case RSXVertex::VECTOR::MAD: {
                decompiled_src =
                    std::format("(({} * {}) + {})", source(src0, instr), source(src1, instr), source(src2, instr));
                break;
            }
            ...
            case RSXVertex::VECTOR::DPH: {
                decompiled_src =
                    std::format("vec4(dot(vec4({}.xyz, 1.0), {}))", source(src0, instr), source(src1, instr));
                break;
            }
            ...
            case RSXVertex::VECTOR::SGE: {
                decompiled_src =
                    std::format("vec4(greaterThanEqual({}, {}))", source(src0, instr), source(src1, instr));
                break;
            }
        ...
        auto decompiled = [instr](std::string decompiled_dest,
                                  std::string decompiled_src,
                                  std::string mask,
            ...
            if (instr->w0.cond_enable) {
                ...
                static const std::string cond_func[] = {
                    "invalidCond", "lessThan", "equal", "lessThanEqual", "greaterThan", "notEqual", "greaterThanEqual"};

                std::string cond = std::format("{}({}, vec4(0.0f))", cond_func[instr->w0.cond], cond_reg);
                decompiled_src =
                    std::format("mix({}{}, {}{}, {}{})", decompiled_dest, mask, decompiled_src, mask, cond, mask);
...
void VertexShaderDecompiler::markOutputAsUsed(std::string name, int location) {
    if (used_outputs[location])
        return;
    used_outputs[location] = true;
    if (location != 0)
        outputs +=
            "layout (location = " + std::to_string(output_locations_map[location]) + ") out vec4 " + name + ";\n";
    else
        outputs += "vec4 " + name + ";\n";
...
std::string VertexShaderDecompiler::source(SourceIndexPair& src_idx, VertexInstruction* instr) {
    ...
    switch (idx) {
        case 0:
            abs = instr->w0.src0_abs;
            break;
        case 1:
            abs = instr->w0.src1_abs;
            break;
        case 2:
            abs = instr->w0.src2_abs;
            break;
    }
