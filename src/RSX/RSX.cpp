...
RSX::RSX(PlayStation3* ps3) : ps3(ps3), gcm(ps3->module_manager.cellGcmSys), fragment_shader_decompiler(ps3) {
    ...
    for (auto& should_flip_tex : should_flip_textures)
...
u32 RSX::offsetAndLocationToAddress(u32 offset, u8 location) {
    if (location == 0)
        return ps3->module_manager.cellGcmSys.gcm_config.local_addr + offset;
    else
        return ioToEa(offset);
}

void RSX::compileProgram() {
    RSXCache::CachedShader cached_shader;
    // Hash the vertex and fragment shaders
    const u64 hash_vertex =
        cache.computeHash((u8*)&vertex_shader_data[vertex_shader_start_idx * 4], 512 * 4 - vertex_shader_start_idx * 4);
    const u64 hash_fragment =
        cache.computeHash(fragment_shader_program.getData(ps3->mem), fragment_shader_program.getSize(ps3->mem));
...
void RSX::setupVAO() {
    ...
    for (auto& binding : vertex_array.bindings) {
        ...
        switch (binding.type) {
            ...
            case 2:
                ...
                break;
            case 3:
                vao.setAttributeFloat<float /* ignored */, true>(
                    binding.index, binding.size, vert_size, (void*)curr_offs, false);
                break;
            case 4:
    ...
    for (auto& binding : immediate_data.bindings) {
        ...
        if (vertex_array.bindings[binding.index].size) {
            // TODO: Is it correct to assume that if an attribute is enabled in the vertex array then we shouldn't use
            // the immediate data? GTAV does this. It writes 0 to all bindings via the SET_VERTEX_DATA_* commands, but
            // then it enables some bindings in the vertex array. Without this check my code would think both the normal
            // vertex array binding and the immediate binding for this index are enabled (which breaks).
            continue;
        }
...
void RSX::uploadVertexConstants() {
    ...
    if (viewport_offs_dirty || program_changed) {
        ...
        glUniform3f(glGetUniformLocation(program.handle(), "viewport_offs"),
                    viewport_offs[0],
                    viewport_offs[1],
                    viewport_offs[2]);
    }
    if (viewport_scale_dirty || program_changed) {
        ...
        glUniform3f(glGetUniformLocation(program.handle(), "viewport_scale"),
                    viewport_scale[0],
                    viewport_scale[1],
                    viewport_scale[2]);
    }
...
void RSX::uploadFragmentUniforms() {
    ...
    for (int i = 0; i < 16; i++) {
        glUniform1i(glGetUniformLocation(program.handle(), std::format("flip_tex{:d}", i).c_str()),
                    should_flip_textures[i] ? GL_TRUE : GL_FALSE);
    }
}
...
void RSX::uploadTexture() {
    ...
    auto swizzle = [this](Texture& texture, bool should_flip_tex) {
        ...
        if (raw_fmt != CELL_GCM_TEXTURE_B8 && raw_fmt != CELL_GCM_TEXTURE_X16 &&
            raw_fmt != CELL_GCM_TEXTURE_X32_FLOAT) {
    ...
    for (int i = 0; i < 16; i++) {
        ...
        // TODO: This will break if a game uploads a different texture but with the same format, width and height to the
        // same address as the previous texture. I'm unsure how common that is. Probably make this toggleable in the
        // future in case some games break
        if (texture == last_tex) {
            continue;
        }
        ...
        // Texture cache
        const u64 hash = cache.computeTextureHash(
            ps3->mem.getPtr(texture.addr), texture.width, texture.height, 4); // TODO: don't hardcode
        if (!cache.getTexture(hash, cached_texture)) {
            ...
            if (!isCompressedFormat(texture.format)) {
                ...
                glTexImage2D(GL_TEXTURE_2D,
                             ...
                             fmt,
                             type,
                             (void*)(!unswizzled_tex ? tex_ptr : unswizzled_tex));
                // checkGLError();
                ...
                glCompressedTexImage2D(GL_TEXTURE_2D,
                                       ...
                                       texture.height,
                                       0,
                                       getCompressedTextureSize(texture.format, texture.width, texture.height),
                                       (void*)ps3->mem.getPtr(texture.addr));
            }
            cache.cacheTexture(hash, cached_texture);
            // lodepng::encode(std::format("./{:08x}.png", texture.addr).c_str(), ps3->mem.getPtr(texture.addr),
...
GLuint RSX::getTextureDataType(u8 fmt) {
    switch (getRawTextureFormat(fmt)) {
        ...
        case CELL_GCM_TEXTURE_R5G5B5A1:
            return GL_UNSIGNED_SHORT_1_5_5_5_REV; // TODO: Unsure if this works
        case CELL_GCM_TEXTURE_A8R8G8B8:
...
GLuint RSX::getPrimitive(u32 prim) {
    switch (prim) {
        ...
        case CELL_GCM_PRIMITIVE_QUAD_STRIP:
            Helpers::panic("Unimplemented quad strip\n");
        case CELL_GCM_PRIMITIVE_POLYGON:
            Helpers::panic("Unimplemented polygon\n");

        default:
            Helpers::panic("Invalid RSX primitive type %d\n", prim);
    }
}
...
bool RSX::doCmd(u32 cmd_num, std::deque<u32>& args) {
    switch (cmd_num) {
        ...
        case NV4097_SET_BLEND_FUNC_SFACTOR: {
            ...
            glBlendFuncSeparate(getBlendFactor(blend_sfactor_rgb),
                                getBlendFactor(blend_dfactor_rgb),
                                getBlendFactor(blend_sfactor_a),
                                getBlendFactor(blend_dfactor_a));
        ...
        case NV4097_SET_BLEND_COLOR: {
            ...
            glBlendColor(
                blend_color_r / 255.0f, blend_color_g / 255.0f, blend_color_b / 255.0f, blend_color_a / 255.0f);
            args.pop_front();
            break;
        }
        ...
        case NV4097_SET_VERTEX_DATA_ARRAY_OFFSET: {
            ...
            log("Vertex attribute %d: offset: 0x%08x\n",
                vertex_array.bindings[idx].index,
                vertex_array.bindings[idx].offset);
            args.pop_front();
            break;
        }
        ...
        case NV4097_SET_VERTEX_DATA_ARRAY_FORMAT: {
            ...
            log("Vertex attribute %d: size: %d, stride: 0x%02x, type: %d\n",
                vertex_array.bindings[idx].index,
                vertex_array.bindings[idx].size,
                vertex_array.bindings[idx].stride,
                vertex_array.bindings[idx].type);
            args.pop_front();
            break;
        }
        ...
        case NV4097_SET_BEGIN_END: {
            ...
            if (prim == 0) { // End
                ...
                if (has_immediate_data) {
                    ...
                    for (auto& binding : immediate_data.bindings) {
                        if (binding.n_verts > 0) { // Binding is active
                            if (binding.n_verts > n_verts)
                                n_verts = binding.n_verts;
                            ...
                            switch (binding.type) {
                                case 1:
                                    vao.setAttributeFloat<GLshort>(
                                        binding.index, binding.size, binding.stride, (void*)old_size, true);
                                    break;
                                case 2:
                                    vao.setAttributeFloat<float>(
                                        binding.index, binding.size, binding.stride, (void*)old_size, false);
                                    break;
                                case 3:
                                    vao.setAttributeFloat<float /* ignored */, true>(
                                        binding.index, binding.size, binding.stride, (void*)old_size, false);
                    ...
                    if (primitive == CELL_GCM_PRIMITIVE_QUADS) {
                        ...
                        glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                                     quad_index_array.size() * 4,
                                     quad_index_array.data(),
                                     GL_STATIC_DRAW);
                ...
                if (inline_array.size()) {
                    ...
                    u32        n_vertices  = 0;
                    for (u32 i = highest_binding->offset - vertex_array.getBase(); i + attrib_size <= n_bytes;
                         i += highest_binding->stride)
                        n_vertices++;
        ...
        case NV4097_DRAW_ARRAYS: {
            ...
            if (primitive == CELL_GCM_PRIMITIVE_QUADS) {
                ...
                glBufferData(
                    GL_ELEMENT_ARRAY_BUFFER, quad_index_array.size() * 4, quad_index_array.data(), GL_STATIC_DRAW);
                glBufferData(GL_ARRAY_BUFFER, vtx_buf.size(), (void*)vtx_buf.data(), GL_STATIC_DRAW);
                glDrawElements(getPrimitive(primitive), quad_index_array.size(), GL_UNSIGNED_INT, 0);
            } else {
                glBufferData(GL_ARRAY_BUFFER, vtx_buf.size(), (void*)vtx_buf.data(), GL_STATIC_DRAW);
        ...
        case NV4097_SET_TRANSFORM_CONSTANT_LOAD: {
            ...
            for (int i = 1; i < args.size(); i++)
                constants[start * 4 + i - 1] = args[i];
            constants_dirty = true;

            log("Upload %d transform constants starting at %d\n", args.size() - 1, args[0]);
            for (int i = 1; i < args.size(); i++) {
                log("0x%08x (%f)\n",
                    constants[start * 4 + i - 1],
                    reinterpret_cast<float&>(constants[start * 4 + i - 1]));
            }
