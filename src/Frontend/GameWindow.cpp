...
#if defined(CHUCKSTATION3_QT_BUILD) && defined(__APPLE__)
...
GameWindow::GameWindow(MainWindow* main_window)
...
void GameWindow::init() {
    window = SDL_CreateWindow("ChuckStation3", 100, 100, 1280, 720, SDL_WINDOW_OPENGL | SDL_WINDOW_SHOWN);
    if (window == nullptr) {
...
void GameWindow::run(PlayStation3* ps3, bool is_rsx_replay) {
    this->ps3         = ps3;
    std::string title = "ChuckStation3";
    ...
    title = std::format("ChuckStation3 | {}", title_game);
#ifndef __APPLE__
...
void GameWindow::pause(bool handle_pc) {
    ...
    // Because the flip is triggered directly by a store instruction to the RSX fifo control, we are running this
    // function before the store instruction ends and PC is incremented. We want to begin stepping from the instruction
    // after, so we increment PC. We decrement it when we unpause because it will be incremented back by the PPU after
    // we return from this function.
    // TODO: This breaks if the instruction that triggered the RSX flip isn't a plain store word. If, for example, it's
    // a store word with update, the "update" part will happen *after* we unpause. Meaning that any instructions we step
    // through while paused won't have the correct state. In 99% of cases it's going to be a plain STW so it's fine, but
    // keep that in mind. This whole thing + the PC incrementing/decrementing won't be an issue when I put the RSX on
    // its own thread (but, I'll have to rethink the way I handle pausing entirely...).
    if (handle_pc)
        ps3->ppu->state.pc += 4;
    ...
    while (true) {
        ...
        ps3->step();
        // Set the "step completed flag". The Qt thread waits on this to know when the step is done, and clears it
        // afterwards
        stepped = true;
    }
...
void GameWindow::updateWindow() {
    ...
    if (curr_time - last_time > 1.0) {
        ...
        title     = std::format("ChuckStation3 | {} | {} FPS | PPU: {:.2f}%",
                            title_game,
                            frame_count,
                            std::ceil(ppu_usage * 100.0f) / 100.0f);
        SDL_SetWindowTitle(window, title.c_str());
        last_time   = curr_time;
        frame_count = 0;
    }
...
void GameWindow::pollInput() {
    ...
    if (!controller) {
        ...
        if (keystate[SDL_SCANCODE_M])
            ps3->pressButton(CELL_PAD_CTRL_START);
        if (keystate[SDL_SCANCODE_K])
            ps3->pressButton(CELL_PAD_CTRL_CROSS);
        if (keystate[SDL_SCANCODE_L])
            ps3->pressButton(CELL_PAD_CTRL_CIRCLE);
        if (keystate[SDL_SCANCODE_J])
            ps3->pressButton(CELL_PAD_CTRL_SQUARE);
        if (keystate[SDL_SCANCODE_I])
            ps3->pressButton(CELL_PAD_CTRL_TRIANGLE);
        if (keystate[SDL_SCANCODE_DOWN])
            ps3->pressButton(CELL_PAD_CTRL_DOWN);
        if (keystate[SDL_SCANCODE_UP])
            ps3->pressButton(CELL_PAD_CTRL_UP);
        if (keystate[SDL_SCANCODE_LEFT])
            ps3->pressButton(CELL_PAD_CTRL_LEFT);
        if (keystate[SDL_SCANCODE_RIGHT])
            ps3->pressButton(CELL_PAD_CTRL_RIGHT);
        ...
        if (SDL_GameControllerGetButton(controller, SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_LEFTSHOULDER))
            ps3->pressButton(CELL_PAD_CTRL_L1);
        if (SDL_GameControllerGetButton(controller, SDL_GameControllerButton::SDL_CONTROLLER_BUTTON_RIGHTSHOULDER))
            ps3->pressButton(CELL_PAD_CTRL_R1);
        if (SDL_GameControllerGetAxis(controller, SDL_GameControllerAxis::SDL_CONTROLLER_AXIS_TRIGGERLEFT) > 20000)
            ps3->pressButton(CELL_PAD_CTRL_L2);
        if (SDL_GameControllerGetAxis(controller, SDL_GameControllerAxis::SDL_CONTROLLER_AXIS_TRIGGERRIGHT) > 20000)
            ps3->pressButton(CELL_PAD_CTRL_R2);
        float leftX = ((float)SDL_GameControllerGetAxis(controller, SDL_GameControllerAxis::SDL_CONTROLLER_AXIS_LEFTX) /
                           32767.0f +
                       1.0f) /
                      2.0f;
        float leftY = ((float)SDL_GameControllerGetAxis(controller, SDL_GameControllerAxis::SDL_CONTROLLER_AXIS_LEFTY) /
                           32767.0f +
                       1.0f) /
                      2.0f;
        float rightX =
            ((float)SDL_GameControllerGetAxis(controller, SDL_GameControllerAxis::SDL_CONTROLLER_AXIS_RIGHTX) /
                 32767.0f +
             1.0f) /
            2.0f;
        float rightY =
            ((float)SDL_GameControllerGetAxis(controller, SDL_GameControllerAxis::SDL_CONTROLLER_AXIS_RIGHTY) /
                 32767.0f +
             1.0f) /
            2.0f;
        // Deadzone
