...
void ThreadDebuggerWidget::update() {
    ...
    for (int i = 0; i < ppu_threads.size(); i++) {
        if (ppu_threads[i].status == Thread::ThreadStatus::Terminated)
            thread_count--;
    }
    ...
    for (int i = 0; i < ppu_threads.size(); i++) {
        ...
        if (thread.status == Thread::ThreadStatus::Terminated)
            continue;
    ...
    ui.spuTable->setHorizontalHeaderLabels(
        QStringList({"Name", "Status", "ID", "pc", "Decrementer", "Enabled Events"}));
    ...
    for (int i = 0; i < spu_threads.size(); i++) {
        ...
        std::string events;
        events.reserve(64);
        if (thread.event_mask.tg)
            events += "tg, ";
        if (thread.event_mask.sn)
            events += "sn, ";
        if (thread.event_mask.reserved)
            events += "reserved, ";
        if (thread.event_mask.qv)
            events += "qv, ";
        if (thread.event_mask.mb)
            events += "mb, ";
        if (thread.event_mask.tm)
            events += "tm, ";
        if (thread.event_mask.me)
            events += "me, ";
        if (thread.event_mask.le)
            events += "le, ";
        if (thread.event_mask.s2)
            events += "s2, ";
        if (thread.event_mask.s1)
            events += "s1, ";
        if (thread.event_mask.lr)
            events += "lr, ";
        if (thread.event_mask.a)
            events += "a, ";
        if (thread.event_mask.ms)
            events += "ms, ";
