...
#ifndef CHUCKSTATION3_USER_BUILD
...
void SPUInterpreter::unimpl(const SPUInstruction& instr) {
    ...
    for (int i = 0; i < 256_KB; i += 4) {
        ...
        out.write((const char*)&val, sizeof(u32));
    }*/
...
void SPUInterpreter::rdch(const SPUInstruction& instr) {
    ...
    // If it did stall, decrease pc so that when the thread wakes up it executes this instruction again and reads the
    // actual value.
    if (ps3->spu_thread_manager.getCurrentThread() == nullptr)
        state.pc -= 4;
    else if (!ps3->spu_thread_manager.getCurrentThread()->isRunning())
        state.pc -= 4;
    should_break = true;
}
...
void SPUInterpreter::bg(const SPUInstruction& instr) {
    state.gprs[instr.rt0].w[0] = state.gprs[instr.rb].w[0] >= state.gprs[instr.ra].w[0];
    state.gprs[instr.rt0].w[1] = state.gprs[instr.rb].w[1] >= state.gprs[instr.ra].w[1];
    state.gprs[instr.rt0].w[2] = state.gprs[instr.rb].w[2] >= state.gprs[instr.ra].w[2];
    state.gprs[instr.rt0].w[3] = state.gprs[instr.rb].w[3] >= state.gprs[instr.ra].w[3];
...
void SPUInterpreter::sfh(const SPUInstruction& instr) {
    for (int i = 0; i < 8; i++)
        state.gprs[instr.rt0].h[i] = state.gprs[instr.rb].h[i] - state.gprs[instr.ra].h[i];
}
...
void SPUInterpreter::rot(const SPUInstruction& instr) {
    state.gprs[instr.rt0].w[0] = std::rotl(state.gprs[instr.ra].w[0], state.gprs[instr.rb].w[0] & 0x1f);
    state.gprs[instr.rt0].w[1] = std::rotl(state.gprs[instr.ra].w[1], state.gprs[instr.rb].w[1] & 0x1f);
    state.gprs[instr.rt0].w[2] = std::rotl(state.gprs[instr.ra].w[2], state.gprs[instr.rb].w[2] & 0x1f);
    state.gprs[instr.rt0].w[3] = std::rotl(state.gprs[instr.ra].w[3], state.gprs[instr.rb].w[3] & 0x1f);
}

void SPUInterpreter::rotm(const SPUInstruction& instr) {
    state.gprs[instr.rt0].w[0] = safeShr<u32>(state.gprs[instr.ra].w[0], (0 - state.gprs[instr.rb].w[0]) & 0x3f);
    state.gprs[instr.rt0].w[1] = safeShr<u32>(state.gprs[instr.ra].w[1], (0 - state.gprs[instr.rb].w[1]) & 0x3f);
    state.gprs[instr.rt0].w[2] = safeShr<u32>(state.gprs[instr.ra].w[2], (0 - state.gprs[instr.rb].w[2]) & 0x3f);
...
void SPUInterpreter::shl(const SPUInstruction& instr) {
    ...
    state.gprs[instr.rt0].w[2] = safeShl<u32>(state.gprs[instr.ra].w[2], state.gprs[instr.rb].w[2] & 0x3f);
    state.gprs[instr.rt0].w[3] = safeShl<u32>(state.gprs[instr.ra].w[3], state.gprs[instr.rb].w[3] & 0x3f);
}
...
void SPUInterpreter::rotmi(const SPUInstruction& instr) {
    const s32 sh               = (0 - ext<s32, 7>(instr.i7)) & 0x3f;
    state.gprs[instr.rt0].w[0] = safeShr<u32>(state.gprs[instr.ra].w[0], sh);
    state.gprs[instr.rt0].w[1] = safeShr<u32>(state.gprs[instr.ra].w[1], sh);
    state.gprs[instr.rt0].w[2] = safeShr<u32>(state.gprs[instr.ra].w[2], sh);
    state.gprs[instr.rt0].w[3] = safeShr<u32>(state.gprs[instr.ra].w[3], sh);
}

void SPUInterpreter::shlhi(const SPUInstruction& instr) {
    const s32 sh = ext<s32, 7>(instr.i7) & 0x1f;
    for (int i = 0; i < 8; i++)
        state.gprs[instr.rt0].h[i] = safeShl<u16>(state.gprs[instr.ra].h[i], sh);
}

void SPUInterpreter::rotmai(const SPUInstruction& instr) {
    const s32 sh = (0 - ext<s32, 7>(instr.i7)) & 0x3f;
    state.gprs[instr.rt0].w[0] =
        isShOK<s32>(sh) ? (s32)state.gprs[instr.ra].w[0] >> sh : (s32)state.gprs[instr.ra].w[0] >> 31;
    state.gprs[instr.rt0].w[1] =
        isShOK<s32>(sh) ? (s32)state.gprs[instr.ra].w[1] >> sh : (s32)state.gprs[instr.ra].w[1] >> 31;
    state.gprs[instr.rt0].w[2] =
        isShOK<s32>(sh) ? (s32)state.gprs[instr.ra].w[2] >> sh : (s32)state.gprs[instr.ra].w[2] >> 31;
    state.gprs[instr.rt0].w[3] =
        isShOK<s32>(sh) ? (s32)state.gprs[instr.ra].w[3] >> sh : (s32)state.gprs[instr.ra].w[3] >> 31;
}

void SPUInterpreter::shli(const SPUInstruction& instr) {
    const s32 sh               = instr.i7 & 0x3f;
    state.gprs[instr.rt0].w[0] = safeShl<u32>(state.gprs[instr.ra].w[0], sh);
    state.gprs[instr.rt0].w[1] = safeShl<u32>(state.gprs[instr.ra].w[1], sh);
    state.gprs[instr.rt0].w[2] = safeShl<u32>(state.gprs[instr.ra].w[2], sh);
    state.gprs[instr.rt0].w[3] = safeShl<u32>(state.gprs[instr.ra].w[3], sh);
}

void SPUInterpreter::rothmi(const SPUInstruction& instr) {
    const s32 sh = (0 - ext<s32, 7>(instr.i7)) & 0x1f;
    for (int i = 0; i < 8; i++)
        state.gprs[instr.rt0].h[i] = safeShr<u16>(state.gprs[instr.ra].h[i], sh);
}
...
void SPUInterpreter::biz(const SPUInstruction& instr) {
    if (state.gprs[instr.rt0].w[3] == 0) {
        ...
        state.pc       = addr - 4;
        ps3->spu_thread_manager.getCurrentThread()->maybeEnableOrDisableInterrupts(instr);
    }
}
...
void SPUInterpreter::iret(const SPUInstruction& instr) {
...
void SPUInterpreter::gb(const SPUInstruction& instr) {
    GPR tmp = state.gprs[instr.ra];
    clr(state.gprs[instr.rt0]);
    state.gprs[instr.rt0].w[3] = (tmp.w[0] & 1) << 0;
    state.gprs[instr.rt0].w[3] |= (tmp.w[1] & 1) << 1;
    state.gprs[instr.rt0].w[3] |= (tmp.w[2] & 1) << 2;
    state.gprs[instr.rt0].w[3] |= (tmp.w[3] & 1) << 3;
}
...
void SPUInterpreter::gbh(const SPUInstruction& instr) {
    ...
    clr(state.gprs[instr.rt0]);
    for (int i = 0; i < 8; i++)
        state.gprs[instr.rt0].w[3] |= (tmp.h[i] & 1) << i;
}
...
void SPUInterpreter::gbb(const SPUInstruction& instr) {
    ...
    clr(state.gprs[instr.rt0]);
    for (int i = 0; i < 16; i++)
        state.gprs[instr.rt0].w[3] |= (tmp.b[i] & 1) << i;
}
...
void SPUInterpreter::frest(const SPUInstruction& instr) {
...
void SPUInterpreter::frsqest(const SPUInstruction& instr) {
    ...
    for (int i = 0; i < 4; i++) {
        const float val = state.gprs[instr.ra].f[i];
        if (exp(val) == 0)
            printf("WARNING! frqest: zero exponent (val = %f, exp = %d)\n", val, exp(val));
...
void SPUInterpreter::rotqmbybi(const SPUInstruction& instr) {
    const s32  sh   = (0 - (state.gprs[instr.rb].w[3] >> 3)) & 0x1f;
    const auto temp = state.gprs[instr.ra];
    clr(state.gprs[instr.rt0]);
    for (int i = 0; i < 16; i++)
...
void SPUInterpreter::shlqby(const SPUInstruction& instr) {
    const s32  sh   = state.gprs[instr.rb].w[3] & 0x1f;
    const auto temp = state.gprs[instr.ra];
    clr(state.gprs[instr.rt0]);
    for (int i = sh; i < 16; i++)
        state.gprs[instr.rt0].b[i] = temp.b[i - sh];
...
void SPUInterpreter::orx(const SPUInstruction& instr) {
    state.gprs[instr.rt0].w[3] =
        state.gprs[instr.ra].w[0] | state.gprs[instr.ra].w[1] | state.gprs[instr.ra].w[2] | state.gprs[instr.ra].w[3];
    state.gprs[instr.rt0].w[2] = 0;
    state.gprs[instr.rt0].w[1] = 0;
    state.gprs[instr.rt0].w[0] = 0;
}
...
void SPUInterpreter::cgth(const SPUInstruction& instr) {
    for (int i = 0; i < 8; i++)
        state.gprs[instr.rt0].h[i] = ((s16)state.gprs[instr.ra].h[i] > (s16)state.gprs[instr.rb].h[i]) ? 0xffff : 0;
}
...
void SPUInterpreter::sumb(const SPUInstruction& instr) {
    for (int i = 0; i < 4; i++) {
        const u32 h = state.gprs[instr.rb].b[(i * 4) + 0] + state.gprs[instr.rb].b[(i * 4) + 1] +
                      state.gprs[instr.rb].b[(i * 4) + 2] + state.gprs[instr.rb].b[(i * 4) + 3];
        const u32 l = state.gprs[instr.ra].b[(i * 4) + 0] + state.gprs[instr.ra].b[(i * 4) + 1] +
                      state.gprs[instr.ra].b[(i * 4) + 2] + state.gprs[instr.ra].b[(i * 4) + 3];
...
void SPUInterpreter::fa(const SPUInstruction& instr) {
    ...
    state.gprs[instr.rt0].f[2] = state.gprs[instr.ra].f[2] + state.gprs[instr.rb].f[2];
    state.gprs[instr.rt0].f[3] = state.gprs[instr.ra].f[3] + state.gprs[instr.rb].f[3];
}

void SPUInterpreter::fs(const SPUInstruction& instr) {
    state.gprs[instr.rt0].f[0] = state.gprs[instr.ra].f[0] - state.gprs[instr.rb].f[0];
    state.gprs[instr.rt0].f[1] = state.gprs[instr.ra].f[1] - state.gprs[instr.rb].f[1];
    state.gprs[instr.rt0].f[2] = state.gprs[instr.ra].f[2] - state.gprs[instr.rb].f[2];
    state.gprs[instr.rt0].f[3] = state.gprs[instr.ra].f[3] - state.gprs[instr.rb].f[3];
...
void SPUInterpreter::fm(const SPUInstruction& instr) {
    state.gprs[instr.rt0].f[0] = state.gprs[instr.ra].f[0] * state.gprs[instr.rb].f[0];
    state.gprs[instr.rt0].f[1] = state.gprs[instr.ra].f[1] * state.gprs[instr.rb].f[1];
    state.gprs[instr.rt0].f[2] = state.gprs[instr.ra].f[2] * state.gprs[instr.rb].f[2];
    state.gprs[instr.rt0].f[3] = state.gprs[instr.ra].f[3] * state.gprs[instr.rb].f[3];
...
void SPUInterpreter::fcmgt(const SPUInstruction& instr) {
    state.gprs[instr.rt0].w[0] =
        (std::abs(state.gprs[instr.ra].f[0]) > std::abs(state.gprs[instr.rb].f[0])) ? 0xffffffff : 0;
    state.gprs[instr.rt0].w[1] =
        (std::abs(state.gprs[instr.ra].f[1]) > std::abs(state.gprs[instr.rb].f[1])) ? 0xffffffff : 0;
    state.gprs[instr.rt0].w[2] =
        (std::abs(state.gprs[instr.ra].f[2]) > std::abs(state.gprs[instr.rb].f[2])) ? 0xffffffff : 0;
    state.gprs[instr.rt0].w[3] =
        (std::abs(state.gprs[instr.ra].f[3]) > std::abs(state.gprs[instr.rb].f[3])) ? 0xffffffff : 0;
}
...
void SPUInterpreter::dfs(const SPUInstruction& instr) {
    state.gprs[instr.rt0].d[0] = state.gprs[instr.ra].d[0] - state.gprs[instr.rb].d[0];
    state.gprs[instr.rt0].d[1] = state.gprs[instr.ra].d[1] - state.gprs[instr.rb].d[1];
}

void SPUInterpreter::dfm(const SPUInstruction& instr) {
    state.gprs[instr.rt0].d[0] = state.gprs[instr.ra].d[0] * state.gprs[instr.rb].d[0];
    state.gprs[instr.rt0].d[1] = state.gprs[instr.ra].d[1] * state.gprs[instr.rb].d[1];
}
...
void SPUInterpreter::addx(const SPUInstruction& instr) {
    state.gprs[instr.rt0].w[0] =
        state.gprs[instr.ra].w[0] + state.gprs[instr.rb].w[0] + (state.gprs[instr.rt0].w[0] & 1);
    state.gprs[instr.rt0].w[1] =
        state.gprs[instr.ra].w[1] + state.gprs[instr.rb].w[1] + (state.gprs[instr.rt0].w[1] & 1);
    state.gprs[instr.rt0].w[2] =
        state.gprs[instr.ra].w[2] + state.gprs[instr.rb].w[2] + (state.gprs[instr.rt0].w[2] & 1);
    state.gprs[instr.rt0].w[3] =
        state.gprs[instr.ra].w[3] + state.gprs[instr.rb].w[3] + (state.gprs[instr.rt0].w[3] & 1);
}
...
void SPUInterpreter::fesd(const SPUInstruction& instr) {
    state.gprs[instr.rt0].d[0] = (double)state.gprs[instr.ra].f[1];
    state.gprs[instr.rt0].d[1] = (double)state.gprs[instr.ra].f[3];
}

void SPUInterpreter::frds(const SPUInstruction& instr) {
    clr(state.gprs[instr.rt0]);
    state.gprs[instr.rt0].f[1] = (float)state.gprs[instr.ra].d[0];
    state.gprs[instr.rt0].f[3] = (float)state.gprs[instr.ra].d[1];
}
...
void SPUInterpreter::mpy(const SPUInstruction& instr) {
    state.gprs[instr.rt0].w[0] =
        (s32)(s16)(state.gprs[instr.ra].w[0] & 0xffff) * (s32)(s16)(state.gprs[instr.rb].w[0] & 0xffff);
    state.gprs[instr.rt0].w[1] =
        (s32)(s16)(state.gprs[instr.ra].w[1] & 0xffff) * (s32)(s16)(state.gprs[instr.rb].w[1] & 0xffff);
    state.gprs[instr.rt0].w[2] =
        (s32)(s16)(state.gprs[instr.ra].w[2] & 0xffff) * (s32)(s16)(state.gprs[instr.rb].w[2] & 0xffff);
    state.gprs[instr.rt0].w[3] =
        (s32)(s16)(state.gprs[instr.ra].w[3] & 0xffff) * (s32)(s16)(state.gprs[instr.rb].w[3] & 0xffff);
}

void SPUInterpreter::mpyh(const SPUInstruction& instr) {
    state.gprs[instr.rt0].w[0] =
        ((s32)(s16)(state.gprs[instr.ra].w[0] >> 16) * (s32)(s16)(state.gprs[instr.rb].w[0] & 0xffff)) << 16;
    state.gprs[instr.rt0].w[1] =
        ((s32)(s16)(state.gprs[instr.ra].w[1] >> 16) * (s32)(s16)(state.gprs[instr.rb].w[1] & 0xffff)) << 16;
    state.gprs[instr.rt0].w[2] =
        ((s32)(s16)(state.gprs[instr.ra].w[2] >> 16) * (s32)(s16)(state.gprs[instr.rb].w[2] & 0xffff)) << 16;
    state.gprs[instr.rt0].w[3] =
        ((s32)(s16)(state.gprs[instr.ra].w[3] >> 16) * (s32)(s16)(state.gprs[instr.rb].w[3] & 0xffff)) << 16;
}

void SPUInterpreter::mpyhh(const SPUInstruction& instr) {
    state.gprs[instr.rt0].w[0] =
        (s32)(s16)(state.gprs[instr.ra].w[0] >> 16) * (s32)(s16)(state.gprs[instr.rb].w[0] >> 16);
    state.gprs[instr.rt0].w[1] =
        (s32)(s16)(state.gprs[instr.ra].w[1] >> 16) * (s32)(s16)(state.gprs[instr.rb].w[1] >> 16);
    state.gprs[instr.rt0].w[2] =
        (s32)(s16)(state.gprs[instr.ra].w[2] >> 16) * (s32)(s16)(state.gprs[instr.rb].w[2] >> 16);
    state.gprs[instr.rt0].w[3] =
        (s32)(s16)(state.gprs[instr.ra].w[3] >> 16) * (s32)(s16)(state.gprs[instr.rb].w[3] >> 16);
}

void SPUInterpreter::mpys(const SPUInstruction& instr) {
    state.gprs[instr.rt0].w[0] =
        ((s32)(s16)(state.gprs[instr.ra].w[0] & 0xffff) * (s32)(s16)(state.gprs[instr.rb].w[0] & 0xffff)) >> 16;
    state.gprs[instr.rt0].w[1] =
        ((s32)(s16)(state.gprs[instr.ra].w[1] & 0xffff) * (s32)(s16)(state.gprs[instr.rb].w[1] & 0xffff)) >> 16;
    state.gprs[instr.rt0].w[2] =
        ((s32)(s16)(state.gprs[instr.ra].w[2] & 0xffff) * (s32)(s16)(state.gprs[instr.rb].w[2] & 0xffff)) >> 16;
    state.gprs[instr.rt0].w[3] =
        ((s32)(s16)(state.gprs[instr.ra].w[3] & 0xffff) * (s32)(s16)(state.gprs[instr.rb].w[3] & 0xffff)) >> 16;
}
...
void SPUInterpreter::mpyu(const SPUInstruction& instr) {
    state.gprs[instr.rt0].w[0] = (u32)(u16)state.gprs[instr.ra].w[0] * (u32)(u16)state.gprs[instr.rb].w[0];
    state.gprs[instr.rt0].w[1] = (u32)(u16)state.gprs[instr.ra].w[1] * (u32)(u16)state.gprs[instr.rb].w[1];
    state.gprs[instr.rt0].w[2] = (u32)(u16)state.gprs[instr.ra].w[2] * (u32)(u16)state.gprs[instr.rb].w[2];
    state.gprs[instr.rt0].w[3] = (u32)(u16)state.gprs[instr.ra].w[3] * (u32)(u16)state.gprs[instr.rb].w[3];
}
...
void SPUInterpreter::fi(const SPUInstruction& instr) {
    // Works if the result from frest is exact and not an estimate (which it is)
    state.gprs[instr.rt0] = state.gprs[instr.rb];
}
...
void SPUInterpreter::cflts(const SPUInstruction& instr) {
    const u32 scale = 1 << (173 - instr.i8);
    for (int i = 0; i < 4; i++) {
        s32 val = (s32)(state.gprs[instr.ra].f[i] * scale);
        if (val > 0x7fffffff)
            val = 0x7fffffff;
        else if (val < -0x80000000)
            val = -0x80000000;
        state.gprs[instr.rt0].w[i] = val;
    }
}
...
void SPUInterpreter::brasl(const SPUInstruction& instr) {
    const u32 addr = ext<s32, 18>(instr.i16 << 2) & 0x3fffc;
    clr(state.gprs[instr.rt0]);
    state.gprs[instr.rt0].w[3] = state.pc + 4;
    state.pc                   = addr - 4;
...
void SPUInterpreter::mpyi(const SPUInstruction& instr) {
    const s16 val              = ext<s16, 10>(instr.si10);
    state.gprs[instr.rt0].w[0] = (s32)(s16)state.gprs[instr.ra].w[0] * (s32)(s16)val;
    state.gprs[instr.rt0].w[1] = (s32)(s16)state.gprs[instr.ra].w[1] * (s32)(s16)val;
    state.gprs[instr.rt0].w[2] = (s32)(s16)state.gprs[instr.ra].w[2] * (s32)(s16)val;
    state.gprs[instr.rt0].w[3] = (s32)(s16)state.gprs[instr.ra].w[3] * (s32)(s16)val;
}

void SPUInterpreter::mpyui(const SPUInstruction& instr) {
    const s16 val              = ext<s16, 10>(instr.si10);
    state.gprs[instr.rt0].w[0] = (u32)(u16)state.gprs[instr.ra].w[0] * (u32)(u16)val;
    state.gprs[instr.rt0].w[1] = (u32)(u16)state.gprs[instr.ra].w[1] * (u32)(u16)val;
    state.gprs[instr.rt0].w[2] = (u32)(u16)state.gprs[instr.ra].w[2] * (u32)(u16)val;
    state.gprs[instr.rt0].w[3] = (u32)(u16)state.gprs[instr.ra].w[3] * (u32)(u16)val;
}
...
void SPUInterpreter::selb(const SPUInstruction& instr) {
    state.gprs[instr.rt].dw[0] = (state.gprs[instr.ra].dw[0] & ~state.gprs[instr.rc].dw[0]) |
                                 (state.gprs[instr.rb].dw[0] & state.gprs[instr.rc].dw[0]);
    state.gprs[instr.rt].dw[1] = (state.gprs[instr.ra].dw[1] & ~state.gprs[instr.rc].dw[1]) |
                                 (state.gprs[instr.rb].dw[1] & state.gprs[instr.rc].dw[1]);
}

void SPUInterpreter::shufb(const SPUInstruction& instr) {
    u8 src[32];
    for (int i = 0; i < 16; i++)
        src[i + 0] = state.gprs[instr.rb].b[i];
    for (int i = 0; i < 16; i++)
        src[i + 16] = state.gprs[instr.ra].b[i];

    for (int i = 0; i < 16; i++) {
        const u8 b = state.gprs[instr.rc].b[i];
        if ((b >> 6) == 0b10)
            state.gprs[instr.rt].b[i] = 0;
        else if ((b >> 5) == 0b110)
            state.gprs[instr.rt].b[i] = 0xff;
        else if ((b >> 5) == 0b111)
            state.gprs[instr.rt].b[i] = 0x80;
        else
            state.gprs[instr.rt].b[i] = src[31 - (b & 0x1f)];
    }
}

void SPUInterpreter::mpya(const SPUInstruction& instr) {
    state.gprs[instr.rt].w[0] =
        ((s32)(s16)(state.gprs[instr.ra].w[0] & 0xffff) * (s32)(s16)(state.gprs[instr.rb].w[0] & 0xffff)) +
        state.gprs[instr.rc].w[0];
    state.gprs[instr.rt].w[1] =
        ((s32)(s16)(state.gprs[instr.ra].w[1] & 0xffff) * (s32)(s16)(state.gprs[instr.rb].w[1] & 0xffff)) +
        state.gprs[instr.rc].w[1];
    state.gprs[instr.rt].w[2] =
        ((s32)(s16)(state.gprs[instr.ra].w[2] & 0xffff) * (s32)(s16)(state.gprs[instr.rb].w[2] & 0xffff)) +
        state.gprs[instr.rc].w[2];
    state.gprs[instr.rt].w[3] =
        ((s32)(s16)(state.gprs[instr.ra].w[3] & 0xffff) * (s32)(s16)(state.gprs[instr.rb].w[3] & 0xffff)) +
        state.gprs[instr.rc].w[3];
}
