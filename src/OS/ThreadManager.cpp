...
Thread* ThreadManager::createThread(u64         entry,
                                    u64         stack_size,
                                    u64         arg,
                                    s32         prio,
                                    const u8*   name,
    ...
    threads.push_back({entry,
                       ...
                       prio,
                       name,
                       !is_emulator_thread ? next_thread_id++ : next_emu_thread_id++,
                       tls_vaddr,
                       tls_filesize,
                       tls_memsize,
                       this});
...
void ThreadManager::reschedule() {
    ...
    do {
        ...
        for (int i = 0; i < threads.size() - 1; i++) {
            Thread& t = threads[(curr_thread + 1 + i) % threads.size()]; // +1 because we want to begin searching from
                                                                         // the thread after the current thread
        ...
        //     - If no SPU thread ever wakes up, something is wrong and the emulator will hang.
            ...
            if (!found_low_prio) {
                if (getCurrentThread()->status == Thread::ThreadStatus::Running)
                    break;
