...
u64 SysLwCond::sysLwCondCreate() {
    ...
    log("sysLwCondCreate(lwcond_ptr: 0x%08x, lwmutex_ptr: 0x%08x, attr_ptr: 0x%08x)",
        lwcond_ptr,
        lwmutex_ptr,
        attr_ptr);
...
u64 SysLwCond::sysLwCondSignalAll() {
    ...
    LwCond*    cond       = (LwCond*)ps3->mem.getPtr(lwcond_ptr);
    Lv2LwCond* lv2_lwcond = ps3->lv2_obj.get<Lv2LwCond>(cond->id);
    if (!lv2_lwcond->signalAll())
        return ps3->ppu->state.gprs[3]; // Locking failed and this thread went to sleep - do not update R3 yet
