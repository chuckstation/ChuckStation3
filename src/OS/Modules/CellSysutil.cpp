...
u64 CellSysutil::cellSysutilCheckCallback() {
    ...
    for (int i = 0; i < 3; i++) {
        if (callbacks[i].func_ptr) {
            ...
            // ps3->ppu->runFunc(ps3->mem.read<u32>(callbacks[i].func_ptr), ps3->mem.read<u32>(callbacks[i].func_ptr +
            // 4)); log("Done\n");
        }
    }
...
u64 CellSysutil::cellSysutilGetSystemParamInt() {
    ...
    switch (param) {
        case CELL_SYSUTIL_SYSTEMPARAM_ID_LANG:
            val = CELL_SYSUTIL_LANG_ENGLISH_US;
            break;
        case CELL_SYSUTIL_SYSTEMPARAM_ID_ENTER_BUTTON_ASSIGN:
            val = CELL_SYSUTIL_ENTER_BUTTON_ASSIGN_CROSS;
            break;
        case CELL_SYSUTIL_SYSTEMPARAM_ID_DATE_FORMAT:
            val = CELL_SYSUTIL_DATE_FMT_DDMMYYYY;
            break;
        case CELL_SYSUTIL_SYSTEMPARAM_ID_TIME_FORMAT:
            val = CELL_SYSUTIL_TIME_FMT_CLOCK24;
            break;
...
u64 CellSysutil::cellSysutilGetSystemParamString() {
    ...
    Helpers::debugAssert(
        buf_size == size, "cellSysutilGetSystemParamString: invalid buf size %d (expected %d)\n", buf_size, size);
