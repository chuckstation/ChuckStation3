...
u64 CellPngDec::cellPngDecReadHeader() {
    ...
    switch (info->n_components) {
        case 3:
            info->color_space = CELL_PNGDEC_RGB;
            break;
        case 4:
            info->color_space = CELL_PNGDEC_RGBA;
            break;
        default:
            Helpers::panic("pngdec: %d components\n", info->n_components);
    }
...
u64 CellPngDec::cellPngDecSetParameter() {
    ...
    out->output_width_byte =
        (curr_info.width * curr_info.n_components * curr_info.bit_depth) / 8; // Number of bytes in a line
...
void CellPngDec::displayPNGInfo(const LodePNGInfo& info) {
    ...
    for (size_t i = 0; i < info.itext_num; i++) {
        std::cout << "Text: " << info.itext_keys[i] << ", " << info.itext_langtags[i] << ", " << info.itext_transkeys[i]
