...
u64 SysLwMutex::sysLwMutexCreate() {
    ...
    log("sysLwMutexCreate(ptr: 0x%08x, attrib_ptr: 0x%08llx (protocol: 0x%08x, recursive: 0x%08x, name: \"%s\")\n",
        ...
        attrib_ptr,
        (u32)attrib->protocol,
        (u32)attrib->recursive,
        name.c_str());

    Lv2Mutex* lv2_mtx = ps3->lv2_obj.create<Lv2Mutex>();
    lv2_mtx->owner    = -1; // free
    if (attrib->recursive == SYS_SYNC_RECURSIVE)
        lv2_mtx->recursive = true;
    else if (attrib->recursive == SYS_SYNC_NOT_RECURSIVE)
        lv2_mtx->recursive = false;
    else
        Helpers::panic("sysLwMutexCreate: invalid attr->recursive\n");
    ...
    mtx->sleep_queue = lv2_mtx->handle(); // The real CellOS would store actual information here - we can just store our
                                          // mutex id instead (this information is hidden from games anyway)
