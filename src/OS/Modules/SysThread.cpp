...
u64 SysThread::sysPPUThreadCreate() {
    ...
    log("sysPPUThreadCreate(thread_id_ptr: 0x%08x, entry: 0x%08x, arg: 0x%016llx, prio: %d, stack_size: 0x%08x, flags: "
        "0x%016llx, thread_name_ptr: 0x%08x)\n",
        thread_id_ptr,
        entry,
        arg,
        prio,
    ...
    Thread* thread = ps3->thread_manager.createThread(entry,
                                                      stack_size,
                                                      arg,
                                                      prio,
                                                      (const u8*)name.c_str(),
                                                      ps3->thread_manager.tls_vaddr,
                                                      ps3->thread_manager.tls_filesize,
                                                      ps3->thread_manager.tls_memsize);
...
u64 SysThread::sysPPUThreadInitializeTLS() {
    ...
    log("sysPPUThreadInitializeTLS(thread_id: %lld, tls_seg_addr: 0x%08x, tls_seg_size: 0x%08x, tls_mem_size: 0x%08x)",
        thread_id,
        tls_seg_addr,
        tls_seg_size,
        tls_mem_size);
...
void SysThread::initializeTLS(
    u64 thread_id, u32 tls_seg_addr, u32 tls_seg_size, u32 tls_mem_size, PPUTypes::State& state) {
    // Allocate TLS memory
    ...
    state.gprs[13] =
        tls_addr + 0x7000 + 0x30; // TODO: figure out why you have to add 0x7000... but stuff breaks without it. I know
                                  // that 0x30 is because the first 0x30 are some sort of reserved system area.
}
