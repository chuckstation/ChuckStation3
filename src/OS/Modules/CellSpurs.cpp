...
u64 CellSpurs::_cellSpursTasksetAttributeInitialize() {
    ...
    log("_cellSpursTasksetAttributeInitialize(attr_ptr: 0x%08x, revision: 0x%08x, sdk_ver: 0x%08x, args: 0x%016llx, "
        "prio_ptr: 0x%08x, max_contention: %d) UNIMPLEMENTED\n",
        attr_ptr,
        revision,
        sdk_ver,
        args,
...
u64 CellSpurs::cellSpursWorkloadAttributeSetName() {
    ...
    log("cellSpursWorkloadAttributeSetName(attr_ptr: 0x%08x, name_class_ptr: 0x%08x, name_instance_ptr: 0x%08x) "
        "[name_class: %s, name_instance: %s] UNIMPLEMENTED\n",
        attr_ptr,
        name_class_ptr,
        name_instance_ptr,
        name_class.c_str(),
...
u64 CellSpurs::_cellSpursEventFlagInitialize() {
    ...
    log("_cellSpursEventFlagInitialize(spurs_ptr: 0x%08x, taskset_ptr: 0x%08x, event_flag_ptr: 0x%08x, "
        "flag_clear_mode: 0x%08x, flag_dir: 0x%08x) UNIMPLEMENTED\n",
        spurs_ptr,
        taskset_ptr,
        event_flag_ptr,
        flag_clear_mode,
...
u64 CellSpurs::cellSpursAttributeEnableSystemWorkload() {
    ...
    log("cellSpursAttributeEnableSystemWorkload(attr_ptr: 0x%08x, prio_ptr: 0x%08x, max_spu: %d, is_preemptible_ptr: "
        "0x%08x) [prio: %s, is_preemptible: %s] UNIMPLEMENTED\n",
        attr_ptr,
        prio_ptr,
        max_spu,
        is_preemptible_ptr,
        prio.c_str(),
        is_preemptible.c_str());
...
u64 CellSpurs::_cellSpursAttributeInitialize() {
    ...
    log("_cellSpursAttributeInitialize(attr_ptr: 0x%08x, revision: 0x%08x, sdk_ver: 0x%08x, n_spus: 0x%08x, spu_prio: "
        "0x%08x, ppu_prio: 0x%08x, exit_if_no_work: 0x%08x) UNIMPLEMENTED\n",
        attr_ptr,
        revision,
        sdk_ver,
        n_spus,
...
u64 CellSpurs::cellSpursInitialize() {
    ...
    log("cellSpursInitialize(spurs_ptr: 0x%08x, n_spus: %d, spu_prio: %d, ppu_prio: %d, exit_if_no_work: %d) "
        "UNIMPLEMENTED\n",
        spurs_ptr,
        n_spus,
        spu_prio,
...
u64 CellSpurs::cellSpursAttachLv2EventQueue() {
    ...
    log("cellSpursAttachLv2EventQueue(spurs_ptr: 0x%08x, queue_id: %d, port_ptr: 0x%08x, is_dynamic: %d) "
        "UNIMPLEMENTED\n",
        spurs_ptr,
        queue_id,
        port_ptr,
...
u64 CellSpurs::cellSpursCreateTask() {
    ...
    log("cellSpursCreateTask(taskset_ptr: 0x%08x, task_id_ptr: 0x%08x, elf_ptr: 0x%08x, ctx_ptr: 0x%08x, size: 0x%08d, "
        "pattern_ptr: 0x%08x, arg_ptr: 0x%08x) UNIMPLEMENTED\n",
        taskset_ptr,
        task_id_ptr,
        elf_ptr,
        ctx_ptr,
...
u64 CellSpurs::cellSpursAddWorkloadWithAttribute() {
    ...
    const u32 attr_ptr        = ARG2;
    log("cellSpursAddWorkloadWithAttribute(spurs_ptr: 0x%08x, workload_id_ptr: 0x%08x, attr_ptr: 0x%08x) "
        "UNIMPLEMENTED\n",
        spurs_ptr,
        workload_id_ptr,
        attr_ptr);
...
u64 CellSpurs::cellSpursGetWorkloadFlag() {
    ...
    ps3->mem.write<u32>(flag_ptr,
                        0x400); // This should be a pointer to a CellSpursWorkloadFlag I think? It's not the flag itself
...
u64 CellSpurs::_cellSpursWorkloadAttributeInitialize() {
    ...
    log("_cellSpursWorkloadAttributeInitialize(attr_ptr: 0x%08x, revision: 0x%08x, sdk_ver: 0x%08x, pm_ptr: 0x%08x, "
        "size: 0x%08x, data: 0x%016llx, prio_ptr: 0x%08x, min_cnt: %d, max_cnt: ???) UNIMPLEMENTED\n",
        attr_ptr,
        revision,
        sdk_ver,
        pm_ptr,
