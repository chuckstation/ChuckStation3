...
u64 CellSaveData::handleSaveDataOperation(
    fs::path savedata_path, u32 stat_cb_ptr, u32 file_cb_ptr, u32 set_buf_ptr, u32 userdata_ptr) {
    ...
    // file_num contains the total amount of files in the directory, file_list_num contains the number of files in the
    // file list. They can differ because in CellSaveDataSetBuf you can limit the file list size, but we should still
    // report the correct amount of files in file_num.
    stat_get->file_num      = 0;
    ...
    for (auto& i : fs::directory_iterator(ps3->fs.guestPathToHost(savedata_path))) {
        // Skip PARAM.SFO
        if (i.path().filename() == "PARAM.SFO")
            continue;

        stat_get->file_num = stat_get->file_num + 1;

        // Check if we went over the file list limit in the buffer settings
        if (stat_get->file_list_num > set_buf->file_list_max)
            continue; // Don't continue populating the list, but keep updating file_num
    ...
    for (int i = 0; i < stat_get->file_list_num; i++) {
        log("- name: \"%s\" size: %lldkb type %d %s\n",
            ...
            file_list[i].size / 1024,
            (u32)file_list[i].filetype,
            file_list[i].filetype == CELL_SAVEDATA_FILETYPE_SECUREFILE ? "(secure)" : "");
    }
    ...
    if (stat_set->set_param_ptr) {
        ...
        // return CELL_OK;
        Helpers::panic("savedata: new savedata but no set_param specified\n");
    }
    ...
    if (file_cb_ptr) {
        ...
        while (true) {
            std::memset(
                ps3->mem.getPtr(res_ptr), 0, sizeof(CellSaveDataCBResult) - sizeof(u32)); // Don't clear the userdata
            std::memset(ps3->mem.getPtr(file_get_ptr + sizeof(u32)),
                        0,
                        sizeof(CellSaveDataFileGet) - sizeof(u32)); // Don't clear exc_size
            ...
            // Are we done saving?
            if (res->result == CELL_SAVEDATA_CBRESULT_OK_LAST ||
                res->result == CELL_SAVEDATA_CBRESULT_OK_LAST_NOCONFIRM) {
            ...
            switch (file_set->filetype) {
                ...
                case CELL_SAVEDATA_FILETYPE_NORMALFILE:
                    filename = Helpers::readString(ps3->mem.getPtr(file_set->file_name));
                    break;
                default:
                    Helpers::panic("savedata: unimplemented filetype %d\n", (u32)file_set->filetype);
            }
            ...
            switch (file_set->file_op) {
                ...
                default:
                    Helpers::panic("Unimplemented file operation %d\n", (u32)file_set->file_op);
            }
        }
    }
...
u64 CellSaveData::cellSaveDataUserListAutoLoad() {
    ...
    log("cellSaveDataUserListAutoLoad(ver: 0x%08x, uid: %d, err_dialog: 0x%08x, set_list_ptr: 0x%08x, set_buf_ptr: "
        "0x%08x, fixed_cb_ptr: 0x%08x, stat_cb_ptr: 0x%08x, file_cb_ptr: 0x%08x, container: %d, userdata_ptr: "
        "0x%08x)\n",
        ver,
        uid,
        err_dialog,
...
u64 CellSaveData::cellSaveDataUserAutoSave() {
    ...
    log("cellSaveDataUserAutoSave(ver: 0x%08x, uid: %d, dir_ptr: 0x%08x, err_dialog: 0x%08x, set_buf_ptr: 0x%08x, "
        "stat_cb_ptr: 0x%08x, file_cb_ptr: 0x%08x, container: 0x%08x, userdata_ptr: 0x%08x) [dir: %s]\n",
        ver,
        uid,
        dir_ptr,
        err_dialog,
...
u64 CellSaveData::cellSaveDataAutoSave2() {
    ...
    const auto dir          = Helpers::readString(ps3->mem.getPtr(dir_ptr));
    log("cellSaveDataAutoSave2(ver: 0x%08x, dir_ptr: 0x%08x, err_dialog: 0x%08x, set_buf_ptr: 0x%08x, stat_cb_ptr: "
        "0x%08x, file_cb_ptr: 0x%08x, container: 0x%08x, userdata_ptr: 0x%08x) [dir: %s]\n",
        ver,
        dir_ptr,
        err_dialog,
        set_buf_ptr,
...
u64 CellSaveData::cellSaveDataAutoLoad2() {
    ...
    const u32  userdata_ptr = ARG7;
    const auto dir          = Helpers::readString(ps3->mem.getPtr(dir_ptr));
    log("cellSaveDataAutoLoad2(ver: 0x%08x, dir_ptr: 0x%08x, err_dialog: 0x%08x, set_buf_ptr: 0x%08x, stat_cb_ptr: "
        "0x%08x, file_cb_ptr: 0x%08x, container: 0x%08x, userdata_ptr: 0x%08x) [dir: %s]\n",
        ver,
        dir_ptr,
        err_dialog,
