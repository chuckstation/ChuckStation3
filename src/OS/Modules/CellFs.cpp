...
u64 CellFs::cellFsOpendir() {
    ...
    if (path == ".")
        return CELL_ENOENT; // TODO: why?
    if (path == "")
        return CELL_ENOENT; // TLOU does this, it's meant to happen

    if (!ps3->fs.isValidDevice(path))
        return CELL_ENOTMOUNTED;
    if (!ps3->fs.isDeviceMounted(path))
        return CELL_ENOTMOUNTED;
...
u64 CellFs::cellFsRead() {
    ...
    const u64 bytes_read = ps3->fs.read(file_id, buf, size);
    if (bytes_read_ptr) // Note: this behavior is different from sys_fs_read, which returns CELL_EFAULT in case
                        // bytes_read_ptr is null.
        ps3->mem.write<u64>(bytes_read_ptr, bytes_read);
...
u64 CellFs::cellFsGetDirectoryEntries() {
    ...
    for (count = 0; count < entries_size / sizeof(CellFsDirectoryEntry); count++) {
        ...
        if (!fsReadDir(file_id, &dirents[count].entry_name))
            break;
        ...
        bool       is_dir = ps3->fs.isDirectory(path);
        dirents[count].attribute.mode =
            !is_dir ? (CELL_FS_S::CELL_FS_S_IFREG | 0666) : (CELL_FS_S::CELL_FS_S_IFDIR | 0777);
...
u64 CellFs::cellFsWrite() {
    ...
    if (bytes_written_ptr) // Note: this behavior is different from sys_fs_write, which returns CELL_EFAULT in case
                           // bytes_written_ptr is null.
