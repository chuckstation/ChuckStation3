...
fs::path CellSsl::getCertificatePathByID(u32 id) {
    ...
    Helpers::debugAssert(
        ps3->fs.exists(cert_path), "Could not find required certificate %s\n", cert_path.generic_string().c_str());
...
u64 CellSsl::cellSslCertificateLoader() {
    ...
    if (!buf_ptr) {
        ...
        Helpers::debugAssert(size >= buf.length(),
                             "cellSslCertificateLoader(): buf is not big enough to fit certificates\n");
