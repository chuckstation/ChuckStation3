...
u64 CellGcmSys::cellGcmSetUserHandler() {
    const u32 handler_ptr = ARG0;
    log("cellGcmSetUserHandler(handler_ptr: 0x%08x)\n");

    user_handler = handler_ptr;
    return CELL_OK;
}

u64 CellGcmSys::cellGcmSetQueueHandler() {
    const u32 handler_ptr = ARG0;
    log("cellGcmSetQueueHandler(handler_ptr: 0x%08x)\n");
...
u64 CellGcmSys::cellGcmInitBody() {
    ...
    static const u8 code[] = {
        0x39, 0x60, 0x00, 0x8d, 0x38, 0x60, 0x03, 0xe8, /* 0x38, 0x60, 0x01, 0x00, */
        0x44, 0x00, 0x00, 0x02, 0x39, 0x60, 0x10, 0x00, 0x44, 0x00, 0x00, 0x02, 0x4b, 0xff, 0xff, 0xec,
    ...
    auto thread =
        ps3->thread_manager.createThread(vblank_thread_entry, 1, 0, 0, (const u8*)"gcm_vblank_thread", 0, 0, 0);
...
u64 CellGcmSys::cellGcmGetReportDataAddressLocation() {
    ...
    if (loc == 1) { // Main
        return ps3->rsx.ioToEa(0x0e000000 + idx * 16);
    }
...
u64 CellGcmSys::cellGcmSetDisplayBuffer() {
    ...
    CellGcmDisplayInfo* info =
        (CellGcmDisplayInfo*)ps3->mem.getPtr(buffer_info_addr + sizeof(CellGcmDisplayInfo) * buf_id);
    info->offset = offs;
    info->width  = width;
    info->height = height;
    info->pitch  = pitch;
...
u64 CellGcmSys::cellGcmSetTileInfo() {
    ...
    const u16 base        = ARG6;
    const u8  bank        = ARG7;
    log("cellGcmSetTileInfo(idx: %d, location: %d, offs: 0x%08x, size: 0x%08x, pitch: 0x%08x, compression: %d, base: "
        "0x%04x, bank: %d) UNIMPLEMENTED\n",
        idx,
        location,
        offs,
        size,
