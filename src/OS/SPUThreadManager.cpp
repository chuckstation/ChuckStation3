...
void SPUThreadManager::contextSwitch(SPUThread& thread) {
    ...
    if (current_thread && current_thread->id == thread.id)
        return;

    if (current_thread)
        log("Switched from thread %d \"%s\" to thread %d \"%s\"\n",
            current_thread->id,
            current_thread->name.c_str(),
            thread.id,
            thread.name.c_str());
    else
...
void SPUThreadManager::reschedule() {
    ...
    int curr_thread = 0;
    while (threads[curr_thread].id != getCurrentThread()->id)
        curr_thread++;
    ...
    for (int i = 0; i < threads.size(); i++) {
        SPUThread& t =
            threads[(curr_thread + 1 + i) %
                    threads.size()]; // +1 because we want to begin searching from the thread after the current thread
...
void SPUThreadManager::createReservation(u32 addr) {
    ...
    for (u32 curr_addr = addr; curr_addr < addr + 128; curr_addr++) {
        ps3->mem.watchpoints_w[curr_addr] =
            std::bind(&SPUThreadManager::reservationWritten, this, std::placeholders::_1);
        // Slowmem for writes (to trigger the watchpoints)
        // We do this in the loop because the SPU could technically reserve an area that crosses page boundaries
