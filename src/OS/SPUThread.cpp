...
void SPUThread::maybeEnableOrDisableInterrupts(const SPUInstruction& instr) {
    ...
    if (instr.e)        interrupts_enabled = true;
    else if (instr.d)   interrupts_enabled = false;
}
...
u32 SPUThread::readChannel(u32 ch) {
    ...
    case SPU_RdEventMask:   return event_mask.raw;
...
u32 SPUThread::readChannelCount(u32 ch) {
    ...
    case MFC_RdListStallStat:   return stall_stat != 0;
...
void SPUThread::doCmd(u32 cmd) {
    auto crosses = [&](u32 addr, u64 size) -> bool {
        ...
        if (aligned - addr < size) return true;
        return false;
    };
