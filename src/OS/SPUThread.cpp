...
SPUThread::SPUThread(PlayStation3* ps3, std::string name, bool is_raw, int raw_idx)
    : ps3(ps3), is_raw(is_raw), raw_idx(raw_idx) {
...
void SPUThread::init() {
    ...
    if (!is_raw) {
        ...
        // Setup watchpoints
        for (u32 addr = problem_addr; addr <= problem_addr + 0x1c00c;
             addr += 4) { // 0x1c00c is the highest known problem state offset
        ...
        switch (segs[i].type) {
            case SYS_SPU_SEGMENT_TYPE_COPY: {
                ...
                log("*  Copied segment from main[0x%08x - 0x%08x] to ls[0x%08x - 0x%08x]\n",
                    (u32)segs[i].src.addr,
                    segs[i].src.addr + segs[i].size,
                    (u32)segs[i].ls_addr,
                    segs[i].ls_addr + segs[i].size);
                break;
            }
...
void SPUThread::maybeEnableOrDisableInterrupts(const SPUInstruction& instr) {
    Helpers::debugAssert(!(instr.e && instr.d), "SPU: both e and d bits are set");
...
void SPUThread::interruptIfEvent() {
    // TODO: I'm unsure how to handle events triggering interrupts in threads that aren't the current one.
    // I won't have to worry about this when I emulate each thread as a host thread.
    if (hasPendingEvents()) {
        ps3->spu->interrupt();
    }
}
...
u32 SPUThread::readChannel(u32 ch) {
    ...
    switch (ch) {
        ...
        case MFC_RdTagMask:
            return tag_mask;
        case MFC_RdTagStat:
            return tag_mask; // TODO
        case MFC_RdListStallStat: {
            if (!stall_stat)
                Helpers::panic("TODO: RdListStallStat with channel count 0\n");

            const u32 val = stall_stat;
            stall_stat    = 0; // Clear flags
            return val;
        }
        case MFC_RdAtomicStat:
            return atomic_stat;
    ...
    switch (ch) {
        ...
        case SPU_WrOutIntrMbox: {
            ...
            if (spup < 64) {
                ...
                spup &= 63;
                Helpers::debugAssert(
                    ports[spup] != -1, "sys_spu_thread_throw_event: port %d was not connected\n", spup);
...
void SPUThread::doCmd(u32 cmd) {
    auto crosses = [&](u32 addr, u64 size) -> bool {
        auto aligned = ps3->mem.ram.pageAlign(addr);
    ...
    switch (cmd) {
        ...
        case PUTB:
        case PUT: {
            log("PUT 0x%08x 0x%x @ 0x%08x\n", eal, size, ps3->spu->state.pc);
            ps3->mem.copyTo(eal, &ls[lsa & 0x3ffff], size);
            break;
        }
        ...
        case PUTL: {
            ...
            for (int i = 0; i < n_elements; i++) {
                MFCListElement* elem =
                    (MFCListElement*)&ls[(eal & 0x3fff8) +
                                         i * sizeof(MFCListElement)]; // For list commands EAL is an offset in LS
                if (elem->ts) {
        ...
        case GET: {
            log("GET @ 0x%08x\n", ps3->spu->state.pc);
            ps3->mem.copyFrom(&ls[lsa & 0x3ffff], eal, size);
            break;
        }
...
void SPUThread::readProblemState(u32 addr) {
    ...
    switch (offs) {
        ...
        case Prxy_TagStatus_offs: {
            val = tag_mask; // TODO like the rdch TagStat
            break;
        }
...
void SPUThread::writeProblemState(u32 addr) {
    ...
    switch (offs) {
        case MFC_LSA_offs:
            writeChannel(MFC_LSA, val);
            break;
        case MFC_EAH_offs:
            writeChannel(MFC_EAH, val);
            break;
        case MFC_EAL_offs:
            writeChannel(MFC_EAL, val);
            break;
        ...
        case Prxy_QueryType_offs: /* TODO */
            break;
        case Prxy_QueryMask_offs: /* TODO */
            break;
        ...
        case SPU_RunCntl_offs: {
            ...
            switch (request) {
                case 1: {
                    Helpers::debugAssert(!isRunning(),
                                         "Requested Raw SPU thread start, but the thread was already running\n");
                ...
                default:
                    Helpers::panic("Unhandled SPU_RunCntl request %d\n", request);
            }
        ...
        default:
            Helpers::panic(
                "Unhandled problem state register write offset 0x%x (addr: 0x%08x) 0x%08x\n", offs, addr, problem_addr);
    }
}
