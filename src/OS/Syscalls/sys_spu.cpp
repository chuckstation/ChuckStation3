...
u64 Syscall::sys_spu_image_open() {
    ...
    log_sys_spu(
        "sys_spu_image_open(image_ptr: 0x%08x, path_ptr: 0x%08x) [path: %s]\n", image_ptr, path_ptr, path.c_str());
...
u64 Syscall::sys_spu_thread_initialize() {
    ...
    log_sys_spu("sys_spu_thread_initialize(id_ptr: 0x%08x, group_id: %d, spu_num: %d, img_ptr: 0x%08x, attr_ptr: "
                "0x%08x, arg_ptr: 0x%08x)\n",
                id_ptr,
                group_id,
                spu_num,
                img_ptr,
...
u64 Syscall::sys_spu_thread_read_ls() {
    ...
    switch (type) {
        case 1:
            ps3->mem.write<u64>(val_ptr, thread->ls[ls_addr]);
            break;
        case 2:
            ps3->mem.write<u64>(val_ptr, Helpers::bswap<u16>(*(u16*)&thread->ls[ls_addr]));
            break;
        case 4:
            ps3->mem.write<u64>(val_ptr, Helpers::bswap<u32>(*(u32*)&thread->ls[ls_addr]));
            break;
        case 8:
            ps3->mem.write<u64>(val_ptr, Helpers::bswap<u64>(*(u64*)&thread->ls[ls_addr]));
            break;
        default:
            Helpers::panic("sys_spu_thread_read_ls: invalid type %d\n", type);
    }
...
u64 Syscall::sys_spu_thread_group_connect_event() {
    ...
    switch (event_type) {
        case SYS_SPU_THREAD_GROUP_EVENT_RUN:
            group->run_event_queue_id = queue_id;
            break;
        case SYS_SPU_THREAD_GROUP_EVENT_EXCEPTION: /* TODO */
            break;
        case SYS_SPU_THREAD_GROUP_EVENT_SYSTEM_MODULE: /* TODO */
            break;
...
u64 Syscall::sys_spu_thread_connect_event() {
    ...
    const u32 spup      = ARG3;
    log_sys_spu(
        "sys_spu_thread_connect_event(id: %d, equeue_id: %d, etype: 0x%08x, spup: %d)\n", id, equeue_id, etype, spup);
    Helpers::debugAssert(etype == SYS_SPU_THREAD_EVENT_USER,
                         "sys_spu_thread_connect_event: etype != SYS_SPU_THREAD_EVENT_USER\n");

    SPUThread* thread = ps3->spu_thread_manager.getThreadByID(id);
    Helpers::debugAssert(thread != nullptr, "sys_spu_thread_connect_event: bad thread id (%d)\n", id);

    if (thread->ports[spup] != -1)
        Helpers::panic("sys_spu_thread_connect_event: port was already connected\n");
