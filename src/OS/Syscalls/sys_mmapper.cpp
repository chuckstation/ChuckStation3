...
u64 Syscall::sys_mmapper_allocate_address() {
    ...
    log_sys_mmapper(
        "sys_mmapper_allocate_address(size: 0x%016llx, flags: 0x%016llx, alignment: 0x%016llx, addr_ptr: 0x%08x)\n",
        size,
        flags,
        alignment,
        addr_ptr);
...
u64 Syscall::sys_mmapper_allocate_shared_memory() {
    ...
    log_sys_mmapper(
        "sys_mmapper_allocate_shared_memory(ipc_key: 0x%016llx, size: %d, flags: 0x%016llx, handle_ptr: 0x%08x)\n",
        ipc_key,
        size,
        flags,
        handle_ptr);
...
u64 Syscall::sys_mmapper_search_and_map() {
    ...
    const u32 addr_ptr   = ARG3;
    log_sys_mmapper(
        "sys_mmapper_search_and_map(start_addr: 0x%08x, handle: 0x%08x, flags: 0x%016llx, addr_ptr: 0x%08x)\n",
        start_addr,
        handle,
        flags,
        addr_ptr);
