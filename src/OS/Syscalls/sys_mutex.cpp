...
u64 Syscall::sys_mutex_create() {
    ...
    if (attr->recursive == SYS_SYNC_RECURSIVE)
        mutex->recursive = true;
    else if (attr->recursive == SYS_SYNC_NOT_RECURSIVE)
        mutex->recursive = false;
    else
        Helpers::panic("sys_mutex_create: invalid attr->recursive\n");
