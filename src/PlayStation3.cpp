...
PlayStation3::PlayStation3(const fs::path& executable)
    : elf_parser(executable), rsx(this), syscall(this), module_manager(this), thread_manager(this),
      spu_thread_manager(this), prx_manager(this), fs(this), lv2_obj(this, &handle_manager) {
    // Load settings
    settings.load();
...
void PlayStation3::gameSelector() {
    ...
    for (int i = 0; i < game_loader.games.size(); i++) {
        printf("%s\n",
               std::format("[{:>3d}] {} | {:<40s} | {:<40s}",
                           i,
                           game_loader.games[i].id,
                           game_loader.games[i].title,
                           game_loader.games[i].content_path.generic_string())
                   .c_str());
    }
...
int PlayStation3::init() {
    ...
    Thread* main_thread = thread_manager.createThread(entry,
                                                      std::max<u64>(proc_param.primary_stacksize, DEFAULT_STACK_SIZE),
                                                      0,
                                                      proc_param.primary_prio,
                                                      (const u8*)"main",
                                                      elf.tls_vaddr,
                                                      elf.tls_filesize,
                                                      elf.tls_memsize,
    ...
    Thread* idle_thread = thread_manager.createThread(
        idle_thread_entry + 10, DEFAULT_STACK_SIZE, 0, 0, (const u8*)"idle", 0, 0, 0, false, true);
...
static constexpr int reschedule_every_n_cycles = 2048 * 128;
void                 PlayStation3::step() {
    ...
    if (curr_block_cycles > reschedule_every_n_cycles) {
        ...
        if (module_manager.cellGcmSys.ctrl) {
            if (module_manager.cellGcmSys.ctrl->get) {
                rsx.runCommandList();
            }
        }
    }
...
void PlayStation3::flip() {
    ...
    if (module_manager.cellGcmSys.queue_handler) {
        ...
        ppu->runFunc(mem.read<u32>(module_manager.cellGcmSys.queue_handler),
                     mem.read<u32>(module_manager.cellGcmSys.queue_handler + 4));
    ...
    if (module_manager.cellGcmSys.flip_callback) {
        ...
        ppu->runFunc(mem.read<u32>(module_manager.cellGcmSys.flip_callback),
                     mem.read<u32>(module_manager.cellGcmSys.flip_callback + 4));
        ppu->state.gprs[3] = old_r3;
    }
...
void PlayStation3::vblank() {
    if (module_manager.cellGcmSys.vblank_handler) {
        u32 old_r3         = ppu->state.gprs[3];
        ppu->state.gprs[3] = 1; // Handler function is always called with 1 as first argument
        ppu->runFunc(mem.read<u32>(module_manager.cellGcmSys.vblank_handler),
                     mem.read<u32>(module_manager.cellGcmSys.vblank_handler + 4));
        ppu->state.gprs[3] = old_r3;
    }
    ...
    if (module_manager.cellGcmSys.vblank2_handler) {
        ...
        ppu->runFunc(mem.read<u32>(module_manager.cellGcmSys.vblank2_handler),
                     mem.read<u32>(module_manager.cellGcmSys.vblank2_handler + 4));
        ppu->state.gprs[3] = old_r3;
    }
}
    ...
    if (ppu->state.pc == enable_spu_on_pc) {
        for (auto& i : spu_thread_manager.threads) {
            if (i.name == spu_thread_to_enable) {
                i.wakeUp();
                mem.markAsFastMem(
                    enable_spu_on_pc >> PAGE_SHIFT, mem.getPtr(enable_spu_on_pc & ~PAGE_MASK), true, true);
                return;
            }
        }
    }
...
void PlayStation3::createAudioDevice() {
    ...
    if (settings.audio.backend == "Null")
        audio = std::make_unique<NullDevice>();
    else if (settings.audio.backend == "miniaudio")
