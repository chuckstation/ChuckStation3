...
int PlayStation3::init() {
    ...
    Thread* main_thread = thread_manager.createThread(entry, std::max<u64>(proc_param.primary_stacksize, DEFAULT_STACK_SIZE), 0, proc_param.primary_prio, (const u8*)"main", elf.tls_vaddr, elf.tls_filesize, elf.tls_memsize, true, false, elf_path_encrypted);
