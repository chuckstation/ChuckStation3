...
void SPULoader::load(fs::path path, sys_spu_image* img, bool remove) {
    ...
    for (int i = 0; i < elf.segments.size(); i++) {
        ...
        log("* Segment %d type %s: 0x%016llx -> 0x%016llx\n",
            i,
            ELFLoader::segment_type_string[seg->get_type()].c_str(),
        ...
        if (seg->get_type() == PT_LOAD) {
            ...
            if (seg->get_file_size()) {
                ...
                std::memset(ps3->mem.getPtr(addr + seg->get_file_size()),
                            0,
                            size - seg->get_file_size()); // Redundant, FILL type segments are for this
