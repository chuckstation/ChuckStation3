            ...
            if (ps3->fs.exists(sfo_path)) {
                ...
                if (sfo.ints["BOOTABLE"] == 1) {
                    const auto title = std::string(
                        reinterpret_cast<const char*>(sfo.strings["TITLE"].c_str())); // Convert u8string to string
...
bool GameLoader::isDiscGameOK() {
    // TODO: more checks
    if (!ps3->fs.exists("/dev_bdvd/PS3_GAME"))
        return false;
    if (!ps3->fs.exists("/dev_bdvd/PS3_GAME/PARAM.SFO"))
        return false;
    return true;
}
...
GameLoader::InstalledGame GameLoader::getDiscGame() {
    if (!isDiscGameOK())
        Helpers::panic("GameLoader: invalid disc game\n");
    ...
    if (sfo.ints["BOOTABLE"] == 1) {
        const auto title =
            std::string(reinterpret_cast<const char*>(sfo.strings["TITLE"].c_str())); // Convert u8string to string
        const auto     id           = std::string(reinterpret_cast<const char*>(sfo.strings["TITLE_ID"].c_str()));
