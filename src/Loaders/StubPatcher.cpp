...
void StubPatcher::patch(u32 addr, bool lle, PlayStation3* ps3) {
    ...
    // We patch LLE stubs with a syscall as well. The syscall will then redirect the PPU to the appropiate function
    // export. While this isn't optimal (we could just patch the stub to directly jump to the function), it allows me to
    // easily track LLE function calls.
    bool stubbed    = false;
    ...
    for (int i = 0; i < 128; i += 4) {
        ...
        if (instr_raw == 0x4e800420) {
            if (!lle) {
                ...
                if (addr_in_r2)
                    sc += 0x2000;
                ps3->mem.write<u32>(addr + i - 4,
                                    0x44000000 | sc); // sc   (we check the low 16 bits to figure out the stub type)
        ...
        // The stubs contain a branch to an other trampoline stub function, which should jump to the actual module
        // function. In these kinds of stubs, the address is stored in R2 instead of R12
    ...
    if (!stubbed)
        Helpers::panic("Couldn't patch stub\n");
}