...
void PKGInstaller::load(const fs::path& path) {
    ...
    plusaes::crypt_ctr((unsigned char*)file_list,
                       sizeof(PKGItemRecord) * header.item_count,
                       npdrm_pkg_ps3_key,
                       16,
                       &header.pkg_data_riv);
...
void PKGInstaller::getFileAsync(const fs::path&           path,
                                const fs::path&           guest_out,
                                std::function<void(bool)> on_complete) {
    std::thread worker = std::thread([=]() { on_complete(getFile(path, guest_out)); });
    worker.detach();
}
...
void PKGInstaller::installAsync(std::function<void(bool)> on_complete, std::function<void(float)> signal_progress) {
    std::thread worker = std::thread([=]() { on_complete(install(signal_progress)); });
    worker.detach();
}
