...
PRXLibraryInfo PRXLoader::load(const fs::path& path, PRXExportTable& exports) {
    ...
    for (int i = 0; i < elf.segments.size(); i++) {
        ...
        log("* Segment %d type %s: 0x%016llx -> 0x%016llx\n",
            i,
            segment_type_string[seg->get_type()].c_str(),
    ...
    for (int i = 0; i < elf.segments.size(); i++) {
        ...
        if (seg->get_type() == SCE_PPURELA) {
            for (int j = 0; j < seg->get_file_size(); j += sizeof(PRXRelocation)) {
                ...
                Helpers::debugAssert(reloc->addr_idx < elf.segments.size(),
                                     "PRXLoader: addr segment idx is out of range\n");
                Helpers::debugAssert(reloc->data_idx < elf.segments.size(),
                                     "PRXLoader: data segment idx is out of range\n");
    ...
    PRXLibrary* lib = (PRXLibrary*)ps3->mem.getPtr(allocations[0] + elf.segments[0]->get_physical_address() -
                                                   elf.segments[0]->get_offset());
    ...
    u32 stop_func     = 0;
    exportModules(
        lib->exports_start, lib->exports_end, exports, lib, &prologue_func, &epilogue_func, &start_func, &stop_func);
    ...
    for (u32 addr = lib->imports_start; addr < lib->imports_end;) {
        ...
        for (int i = 0; i < module->n_funcs; i++) {
            ...
            log("* Imported function: 0x%08x @ 0x%08x \t[%s]\n",
                nid,
                addr,
                ps3->module_manager.getImportName(nid).c_str());
...
void PRXLoader::exportModules(const u32       exports_start,
    ...
    for (u32 addr = exports_start; addr < exports_end; module_idx++) {
        ...
        log("Library %s exports module %s (%d functions, %d variables)\n",
            lib ? (const char*)lib->name : "UNNAMED",
            ps3->mem.getPtr(module->name_ptr),
            (u16)module->n_funcs,
            (u16)module->n_vars);
        // Helpers::debugAssert(module->n_vars == 0, "PRX module: n_vars > 0\n");
        if (module->n_vars != 0)
            log("WARNING: exporting module with variables (unimplemented)\n");
