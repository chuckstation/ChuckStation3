...
int SELFToELF::makeELF(const fs::path& path, const fs::path& out_path) {
    ...
    if (revision == 0x8000 || revision == 0xc000) {
        ...
        const u64 elf_size =
            revision == 0x8000
                ? header.file_size
                : Helpers::bswap<u64>(
                      header.file_size); // I added the assert above so I will remember to verify if this is swapped too
    ...
    if (int e = plusaes::decrypt_cbc(encryption_root_header_data.get(),
                                     enc_root_hdr_size,
                                     klic,
                                     16,
                                     &iv,
                                     encryption_root_header_no_npdrm.get(),
                                     enc_root_hdr_size,
                                     nullptr)) {
        Helpers::panic("AES error (%d)\n", e);
    }
    ...
    plusaes::decrypt_cbc(encryption_root_header_no_npdrm.get(),
                         enc_root_hdr_size,
                         erk,
                         32,
                         &riv,
                         (u8*)&encryption_root_header,
                         enc_root_hdr_size,
                         nullptr);
    ...
    const auto cert_body_size =
        header.file_offset - (sizeof(CFHeader) + header.ext_header_size + sizeof(CFEncryptionRootHeader));
    ...
    std::memcpy((u8*)segment_headers.get(),
                cert_data.get() + sizeof(CertificationHeader),
                sizeof(SegmentCertificationHeader) * n_certs);

    // Copy keys
    const u32 n_keys = certification_header.attr_entry_num * 16;
    auto      keys   = std::make_unique<u8[]>(n_keys);
    std::memcpy(keys.get(),
                cert_data.get() + sizeof(CertificationHeader) + sizeof(SegmentCertificationHeader) * n_certs,
                n_keys);
    ...
    for (int i = 0; i < n_certs; i++) {
        ...
        log("Loading Certification Segment %d (encrypted: %s compressed: %s)\n",
            i,
            seg.enc_algorithm != 1 ? "yes," : "no, ",
            seg.comp_algorithm != 1 ? "yes" : "no");
        ...
        if (seg.enc_algorithm != 1) {
            ...
            Helpers::debugAssert(seg.key_idx < certification_header.attr_entry_num - 1,
                                 "SELFToELF: segment key idx is out of bounds");
            Helpers::debugAssert(seg.iv_idx < certification_header.attr_entry_num - 1,
                                 "SELFToELF: segment iv idx is out of bounds");
...
SELFToELF::SELFKey& SELFToELF::getSELFKey(u32 type, u16 revision, u64 version) {
    ...
    switch (type) {
        case SELF_TYPE_NPDRM:
            return find_key_with_revision(revision, npdrm_keys, 16);
